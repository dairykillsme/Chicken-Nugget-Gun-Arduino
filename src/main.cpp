...
const char *SSID = "THE_WORK_SITE";
const char *PWD = "godilovemondays";
...
const byte IR_PIN = D9; //IR on RX pin
...
//Motor Setup (These are using TX and RX, so Serial must be disabled)
const byte MOTOR_PIN = D10; //motor on TX pin
const byte SERVO_PIN = D4; 
Servo magServo;
const int UNLOCKED_POSITION = 0;
const int LOCKED_POSITION = 80;
int magServoPosition = LOCKED_POSITION;
...
void loop()
{
  ...
  magServo.write(magServoPosition);
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t *payload, size_t length)
{
  ...
  case WStype_TEXT:
    ...
    {
      magServoPosition = UNLOCKED_POSITION;
    }
    else if (webSocketMessage == "MAGLOCKON")
    {
      magServoPosition = LOCKED_POSITION;
    }
    break;
  }
}